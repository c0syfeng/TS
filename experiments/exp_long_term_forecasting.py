...
import warnings
import numpy as np
from layers.Embed import series_decomp,AutoConCI
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def init_AutoCon(self,args):
        ...
        smoother = series_decomp(args.seq_len+1)#去掉趋势
        x = torch.from_numpy(target_data).unsqueeze(0)
        _, target_data = smoother(x)
        target_data = target_data.squeeze(0).numpy()#去掉趋势之后计算
        acf_values = []
        for i_ch in range(target_data.shape[-1]):
            acf_values.append(acf(target_data[..., i_ch], nlags=len(target_data)))#len()是返回第一维度，计算从1到len的ACF
            ...
            for i, (timeindex,batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    for i in range(len(attns)):
                        attns[i] = attns[i][:,:,:12,:12]
                    attns = torch.cat(attns,dim=1)
                    attns = attns.reshape(B*C,T,-1)

                    features = F.normalize(attns, dim=-1)

                    global_pos_labels = timeindex.long()
                    local_loss, global_loss = self.AutoCon_loss(features, global_pos_labels)
                    autocon_loss = (local_loss.reshape(B, C, T//3).mean(dim=2).mean(dim=1) + global_loss.mean(dim=0))/2.0
                    msemean = MSE_loss.mean()
                    autoloss = autocon_loss.mean()
                    loss = msemean + 0.01*autoloss
                ...
                if self.args.use_amp:
                    ...
                    loss.backward()
                    torch.nn.utils.clip_grad_value_(self.model.parameters(), clip_value=0.5)##############################3
                    model_optim.step()
    ...
    def test(self, setting, test=0):
        ...
        np.save(folder_path + f'metrics_{rmse}.npy', np.array([mae, mse, rmse, mape, mspe]))
